...
describe("model metadata", () => {
	it("keeps the Gemini 3.1 Flash Image preview ID as a stable-model alias", () => {
		...
		const expectedMappings = [
			...
			["glacier", "gemini-3.1-flash-image-preview"],
			["google-vertex", "gemini-3.1-flash-image"],
			["quartz", "gemini-3.1-flash-image"],
		];
		...
		expect(previewModel?.providers).toEqual(stableModel?.providers);
	});
