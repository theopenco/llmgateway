...
export function mapModels(
	...
	for (const m of models) {
		...
		for (const p of m.providers) {
			...
			// Note: ModelSelector uses m.id (Gateway ID), not p.externalId (Provider ID)
			...
			// The existing code used providerId/p.externalId.
			// Let's keep p.externalId for now to avoid breaking if p.externalId is expected elsewhere,
