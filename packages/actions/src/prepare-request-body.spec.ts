...
describe("prepareRequestBody - max_tokens forwarding", () => {
	...
	describe("google-ai-studio", () => {
		...
		test("falls back to model maxOutput on Gemini 3 to prevent infinite hangs", async () => {
			// Gemini 3.x hangs when maxOutputTokens is omitted
			// (googleapis/python-genai#2062). Force a sane upper bound from the
			// model registry instead of leaving it unset.
			const requestBody = (await prepareRequestBody(
				"google-ai-studio",
			...
			// gemini-3-pro-preview declares maxOutput: 65536 in the registry.
			expect(requestBody.generationConfig?.maxOutputTokens).toBe(65536);
		});

		test("forwards caller-supplied max_tokens on Gemini 3 instead of falling back", async () => {
			const requestBody = (await prepareRequestBody(
				"google-ai-studio",
				"gemini-3-pro-preview",
				[{ role: "user", content: "Hello!" }],
			...
			expect(requestBody.generationConfig?.maxOutputTokens).toBe(32000);
		});

		test("maps reasoning_effort to thinkingLevel on Gemini 3 (not thinkingBudget)", async () => {
			// Gemini 3 rejects the numeric thinkingBudget; the API expects
			// thinkingLevel ("low" | "high") instead.
			const requestBody = (await prepareRequestBody(
				"google-ai-studio",
				"gemini-3-pro-preview",
				[{ role: "user", content: "Hello!" }],
		...
		test("maps reasoning_effort low/minimal to thinkingLevel=low on Gemini 3", async () => {
			const requestBody = (await prepareRequestBody(
				"google-ai-studio",
				"gemini-3-pro-preview",
				[{ role: "user", content: "Hello!" }],
