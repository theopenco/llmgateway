...
describe("prepareRequestBody - max_tokens forwarding", () => {
	...
	describe("openai (Responses API)", () => {
		...
		test("recovers call_id when a tool result omits tool_call_id", async () => {
			const requestBody = (await prepareRequestBody(
				...responsesArgs([
					{ role: "user", content: "weather in Berlin?" },
					{
		...
		test("matches legacy function-role results by name", async () => {
			const requestBody = (await prepareRequestBody(
				...responsesArgs([
					...
					// Legacy `function` role: no tool_call_id, only name. Matched by name,
					// so the out-of-order result still resolves to the right call.
					{
						role: "function",
						name: "get_time",
						content: JSON.stringify({ time: "20:52" }),
		...
		test("throws OrphanedToolMessageError for a tool result with no preceding call", async () => {
			await expect(
				prepareRequestBody(
					...responsesArgs([
						{ role: "user", content: "hi" },
