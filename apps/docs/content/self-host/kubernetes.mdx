...
Provider keys support comma-separated values for load balancing and optional `__ENTERPRISE` / `__DEVPASS` suffix overrides used only for enterprise-plan or DevPass organizations — see [Enterprise and DevPass provider env overrides](/self-host/docker-compose#enterprise-and-devpass-provider-env-overrides).
