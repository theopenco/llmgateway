...
Provider keys support comma-separated values for load balancing and an optional `__ENTERPRISE` suffix override used only for enterprise-plan organizations — see [Enterprise-only provider keys](/self-host/docker-compose#enterprise-only-provider-keys).
