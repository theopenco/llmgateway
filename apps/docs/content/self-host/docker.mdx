...
LLM Gateway supports multiple API keys per provider for load balancing — provide comma-separated values. See [Docker Compose](/self-host/docker-compose#multiple-api-keys-and-load-balancing) for details. Provider env vars also support optional `__ENTERPRISE` / `__DEVPASS` suffix overrides used only for enterprise-plan or DevPass organizations — see [Enterprise and DevPass provider env overrides](/self-host/docker-compose#enterprise-and-devpass-provider-env-overrides).
