...
LLM Gateway supports multiple API keys per provider for load balancing — provide comma-separated values. See [Docker Compose](/self-host/docker-compose#multiple-api-keys-and-load-balancing) for details. Provider keys also support an optional `__ENTERPRISE` suffix override used only for enterprise-plan organizations — see [Enterprise-only provider keys](/self-host/docker-compose#enterprise-only-provider-keys).
