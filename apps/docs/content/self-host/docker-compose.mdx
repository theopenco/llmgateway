...
Restart the API and gateway containers after changing it. Every replica may use
the same license. The token is verified offline against a public key included in
the images; it is never sent to LLM Gateway. Core features continue working when
the key is missing or invalid. A standard Enterprise license is bound to one
organization. A white-label license permits multiple Enterprise organizations
and unlocks the multi-organization admin dashboard. Expired licenses receive a
seven-day grace period.

After restart, a signed-in administrator can check `GET /admin/license` for the
state, expiry, scope, seat cap, and current distinct-seat usage. For signing-key
rotation, upgrade the API and gateway images so they contain the new public key
before installing a token with the new key ID. Old public keys remain in the
image during the rotation window.
...
The admin dashboard's **Unstable Mappings** page ranks mappings over a bounded
sample of retained request logs. **Ignore expected** excludes client-classified
errors and applies saved error matchers. **Show all errors** includes those
errors in both the ranking and the error details. The time window, log limit,
retried-request filter, and BYOK filter still apply independently.
