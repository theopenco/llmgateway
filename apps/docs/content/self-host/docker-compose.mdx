...
## Enterprise-only provider keys

Any provider API key env var supports an optional enterprise-only override with the `__ENTERPRISE` suffix. When set, requests from organizations on the enterprise plan use it instead of the base key; all other organizations keep using the base key:
...
- The override is optional — enterprise organizations fall back to the base key when it's unset.
- The base key is still what makes a provider available for routing, so set it even when you route all enterprise traffic through a dedicated key.
- Comma-separated values, load balancing, and health-aware routing work exactly like the base key; the enterprise key list gets its own independent health tracking.
- Region-specific overrides compose with the enterprise suffix. For enterprise organizations the lookup order is `{BASE}__ENTERPRISE__{REGION}` → `{BASE}__{REGION}` → `{BASE}__ENTERPRISE` → `{BASE}`.
- Related configuration values (like `LLM_GOOGLE_CLOUD_PROJECT`) are still read from the base vars at the selected key index, so keep an enterprise key list index-compatible with those lists (or use a single value).
