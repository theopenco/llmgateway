...
When upgrading from a release that stored gateway API keys as plaintext, update
the gateway first and wait for it to become healthy before updating the API.
The compatible gateway accepts both legacy plaintext keys and new hash-only
keys; reversing that order can briefly reject keys created during the rollout.
