...
- Credential tokens are encrypted at rest, so `PROVIDER_KEY_ENCRYPTION_KEY` must
  be set on every service.
- Video generation still resolves its credentials from the environment. Keep the
  `LLM_*` variables set for any provider you generate video with.
