...
## Enterprise and DevPass provider env overrides
...
- `__DEVPASS` suffix: used instead of the base var for DevPass (coding plan) organizations
...
# Used instead of the shared key, but only for DevPass organizations
LLM_OPENAI_API_KEY__DEVPASS=sk-devpass-key

# Companion settings can be overridden the same way, e.g. a dedicated
# Google Cloud project and base URL for enterprise traffic
LLM_GOOGLE_CLOUD_PROJECT__ENTERPRISE=enterprise-gcp-project
LLM_OPENAI_BASE_URL__DEVPASS=https://devpass-proxy.internal
```
...
- The base API key is still what makes a provider available for routing, so set it even when you route all enterprise or DevPass traffic through dedicated keys.
...
- Region-specific overrides compose with the variant suffix. For matching organizations the API-key lookup order is `{BASE}__ENTERPRISE__{REGION}` → `{BASE}__{REGION}` → `{BASE}__ENTERPRISE` → `{BASE}` (same pattern with `__DEVPASS`).
- If an organization is both on the enterprise plan and a DevPass org, the enterprise overrides win.
