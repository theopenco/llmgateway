...
strong random value on every service (`openssl rand -base64 32`).

For rotation, prepend the new secret to the comma-separated keyring and retain
the old entries while credentials and hashes still use them. Removing an old
entry makes its provider credentials undecryptable and its remaining hashes
unusable.
