...
`LLM_GOOGLE_CLOUD_PROJECT` is optional for Google Vertex API-key chat requests, which use the projectless Express Mode endpoint when it is unset. Set it for OAuth authentication and Vertex APIs such as embeddings, speech, and video.
