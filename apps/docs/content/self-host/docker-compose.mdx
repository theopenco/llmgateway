...
`LLM_GOOGLE_CLOUD_PROJECT` is optional for Google Vertex API-key requests, which use the projectless publisher-model endpoint when it is unset. Set it for OAuth authentication or to use project-scoped Vertex URLs.
