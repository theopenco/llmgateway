...
Pinning is what the **LLM Gateway** (`llmgateway-providers`) entry is for. Its model IDs use LLM Gateway's [`provider/model` syntax](/features/routing#provider-specific-routing), so picking "Claude Opus 5 (Anthropic)" in the model picker always runs on that upstream deployment — useful for a fixed price, a compliance boundary, or consistent behavior. The **DevPass (LLM Gateway)** entry lists canonical IDs, which the gateway routes to the best available provider automatically.
