...
- **Nano Banana image generation** - Generate images with Gemini 3 Pro Image Preview and save to disk
...
Generate an image using Gemini 3 Pro Image Preview ("Nano Banana") and save it to disk. Tailored for AI coding agents — returns the file path of the saved image and an inline image preview.

**Parameters:**

- `prompt` (string) - Text description of the image to generate
- `save_path` (string, optional) - Directory to save the image (default: current working directory)
- `filename` (string, optional) - Filename for the saved image (default: `nano-banana-{timestamp}.png`)
...
<Callout type="info">
	Unlike `generate-image`, this tool automatically saves images to your file
	system and returns the absolute path — ideal for AI coding agents that need to
	reference generated images in code.
</Callout>
...
<Tabs items={["Claude Code", "Codex", "Cursor", "Other MCP Clients"]}>
  <Tab value="Claude Code">
    ...
    - "Use generate-nano-banana to create a hero image and save it to ./assets"
...
Generate images with Gemini 3 Pro and save them to disk for use in your project:

```
Use generate-nano-banana to create a hero image for my landing page.
Save it to ./public/images with a 16:9 aspect ratio.
```
