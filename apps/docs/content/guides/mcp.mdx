...
By default, `generate-nano-banana` returns images inline without writing to disk. To enable saving generated images locally (e.g., when using Claude Code), set the `UPLOAD_DIR` environment variable on the MCP server.

<Tabs items={["Claude Code", "Codex"]}>
  <Tab value="Claude Code">
  Add `UPLOAD_DIR` to your MCP server configuration in `~/.claude/claude_mcp_settings.json`:
...
```json
{
	"mcpServers": {
		"llmgateway": {
			...
			"env": {
				"UPLOAD_DIR": "/path/to/your/images"
			}
		}
	}
}
```
  ...
  <Tab value="Codex">
  Set `UPLOAD_DIR` as an environment variable before running Codex:

```bash
export UPLOAD_DIR="/path/to/your/images"
```
...
<Callout type="warn">
	On production servers, do **not** set `UPLOAD_DIR`. This ensures no files are
	written to the server filesystem — images are returned inline only.
</Callout>
