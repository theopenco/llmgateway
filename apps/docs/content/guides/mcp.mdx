...
<Tabs items={["Claude Code", "Cursor", "Windsurf", "VS Code", "Other MCP Clients"]}>
  <Tab value="Claude Code">
    ...
    Run the following command in your terminal to add the LLM Gateway MCP server:
    ...
    This registers the MCP server globally (across all projects) using the HTTP transport. You should now see the LLM Gateway tools available in Claude Code.

    <Callout type="info">
      **Alternative: Manual configuration**

      You can also add the MCP server manually by editing your Claude Code configuration file (`~/.claude/claude_desktop_config.json`):
  ...
  <Tab value="Windsurf">
    ...
    Or open the Command Palette (`Cmd/Ctrl + Shift + P`), search for **"MCP: Add Server"**, select **HTTP**, and enter the URL and headers above.
    ...
    Open Cascade and verify the LLM Gateway tools appear. Press the **refresh** button if needed. Try:
  ...
  <Tab value="VS Code">
    ...
    Add the following to your workspace `.vscode/mcp.json` file:
    ...
    ```bash
    code --add-mcp '{"name":"llmgateway","type":"http","url":"https://api.llmgateway.io/mcp","headers":{"Authorization":"Bearer your-api-key-here"}}'
    ```

    <Callout type="info">
      VS Code 1.99+ with GitHub Copilot is required for MCP support. Use the tools in **Agent Mode** within Copilot Chat.
    </Callout>
    ...
    Open Copilot Chat in **Agent Mode**, click the **Select Tools** icon, and verify the LLM Gateway tools appear. Try:
