...
Override the model for any session using `-m` or `--model`:
...
# Combine with provider flag
hermes chat --provider custom --model deepseek-r1
```
...
| `hermes config set`    | Update config values                    |
