...
Then mark onboarding as complete so the CLI uses your config directly without prompting:
...
<Callout type="info">
	Without the `.onboarding_complete` file, Continue CLI shows an onboarding
	wizard on first run even if `config.yaml` already exists. Creating this file
	tells the CLI to skip the wizard and use your config.
</Callout>
...
Alternatively, you can pass the config explicitly (this also skips the onboarding wizard):
