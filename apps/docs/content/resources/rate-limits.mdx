...
All rate-limit response headers describe limits enforced by **LLMGateway**. A gateway rate-limit response can include:
...
- `RateLimit-Limit` / `X-RateLimit-Limit`: Limit for the current gateway window
- `RateLimit-Remaining` / `X-RateLimit-Remaining`: Remaining allowance in that window
- `RateLimit-Reset`: Seconds until the gateway limit resets
- `X-RateLimit-Reset`: Unix timestamp when the gateway limit resets

Successful authenticated responses on limited endpoints also include `RateLimit-Limit` and `RateLimit-Remaining`, so clients can slow down before reaching their gateway limit.
...
- **Watch the headers.** Monitor `RateLimit-Remaining` to back off before you hit the limit.
