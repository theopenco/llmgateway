...
Other `429` sources carry their own header subsets: free-model limits set `RateLimit-Limit` / `RateLimit-Remaining` on every free-model request (plus `Retry-After` and reset headers when limited) without the structured fields, and a provider-cap rejection under `X-No-Fallback` may arrive without the full set.
