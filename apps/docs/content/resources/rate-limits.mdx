...
LLMGateway can enforce per-provider/model request caps (per-minute and per-day windows) on top of the per-endpoint limits. When such a cap is configured, chat responses that go through provider rate-limit accounting carry provider-scoped headers (`X-RateLimit-Limit-Provider`, `X-RateLimit-Remaining-Provider`, plus per-window `…-Provider-RPM` / `…-Provider-RPD` variants) — on successful responses too, not just on 429s. A request rejected early (a pinned provider with `X-No-Fallback` whose cap is already exhausted) may return the `429` without these headers. A capped provider is normally just deprioritized — [routing](/features/routing) picks a provider with remaining budget instead; a `429` is returned only when the request pins the capped provider explicitly with `X-No-Fallback`.

## Rate Limit Headers

When a request is rejected by the [per-organization endpoint limits](#per-organization-endpoint-limits) or the [concurrency limit](#concurrent-request-limits), the `429` response includes both the standard draft `RateLimit-*` headers and the legacy `X-RateLimit-*` variants:
...
When rate limiting is enforced, successful responses on rate-limited endpoints also carry `RateLimit-Limit` and `RateLimit-Remaining` (without the `X-` prefix), so clients can self-throttle before ever hitting a `429`.

Other `429` sources carry their own header subsets: free-model limits set limit/remaining headers on every free-model request (plus retry/reset headers when limited), while a provider-cap rejection under `X-No-Fallback` may arrive without the full set. Always treat `Retry-After` as the primary backoff signal when present.
