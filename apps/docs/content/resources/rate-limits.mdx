...
Dev plans cover text inference only: **chat completions (including the AI SDK protocol), messages, responses, the models list, and moderations**, plus the key and credits endpoints, each at a flat 120 requests-per-minute floor. The other endpoints (embeddings, rerank, OCR, images, speech, transcriptions, videos, realtime) return a `403` on Dev plans and are marked **—** below; realtime is unavailable on Chat plans too.
...
Other `429` sources carry their own header subsets. Free-model limits overwrite `RateLimit-Limit` / `RateLimit-Remaining` (and the `X-RateLimit-*` equivalents) with the free-model budget on every free-model request, adding `Retry-After` and reset headers when limited, while any `RateLimit-Policy` / `RateLimit` structured fields on that response still describe the organization RPM quota. A provider-cap rejection under `X-No-Fallback` may arrive without the full set.
