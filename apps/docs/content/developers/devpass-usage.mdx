...
description: Read DevPass monthly, daily and weekly usage limits from an application using its gateway API key
icon: Gauge
---
...
```json
{
	"data": {
		...
		"devPlanCreditsRemaining": "133.00",
		"devPlanDailyLimit": "14.22",
		"devPlanDailyCreditsUsed": "3.10",
		"devPlanDayResetsAt": "2026-08-22T09:30:00.000Z",
		"devPlanPremiumWeeklyLimit": "18.96",
...
| `devPlanDailyLimit`          | Daily pacing allowance — plan credits usable per rolling 24h   |
| `devPlanDailyCreditsUsed`    | Plan credits used in the current 24-hour window                |
| `devPlanDayResetsAt`         | ISO 8601 reset time, or `null` when no daily window is active  |
...
The pacing window starts with the first request that draws on the plan
allowance and lasts 24 hours; it covers every model. When it expires, the
endpoint returns `"0.00"` for `devPlanDailyCreditsUsed` and `null` for
`devPlanDayResetsAt`; the full daily share is available again, and the next
