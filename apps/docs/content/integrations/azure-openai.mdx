...
description: Connect Azure OpenAI Service to LLM Gateway for enterprise-grade OpenAI models
icon: Cloud
---

import { Step, Steps } from "fumadocs-ui/components/steps";

Azure OpenAI Service provides access to OpenAI's powerful language models through Microsoft's enterprise cloud infrastructure. This guide shows how to create an Azure OpenAI resource, deploy models, and integrate them with LLM Gateway.
...
Azure OpenAI Service provides enterprise-grade access to OpenAI models with enhanced security, compliance, and regional availability. LLM Gateway integrates seamlessly with Azure OpenAI deployments.
...
3. In Azure OpenAI Studio, select **Deployments** from the left sidebar
4. Click **Create new deployment**
5. Configure your deployment:
   - **Model**: Select a model (e.g., gpt-4o, gpt-4o-mini, gpt-4-turbo)
   - **Deployment name**: Enter a name (this must match the model identifier you'll use)
...
- For `gpt-4o` → deployment name should be `gpt-4o`
- For `gpt-4-turbo` → deployment name should be `gpt-4-turbo`
...
**Important**: Keep your API key secure - it provides access to your Azure OpenAI deployments.
...
<Step>
### Add Azure OpenAI Provider Key

1. Click **Add** for **Azure OpenAI**
2. Enter your **API Key** from Azure Portal
3. Enter your **Resource Name** (the name from your Azure OpenAI endpoint URL)
...
Once configured, you can access your Azure OpenAI deployments through LLM Gateway:
...
**Note**: Only models you have deployed in Azure OpenAI Studio will be available. Ensure your deployment names match the expected model identifiers.
...
- Verify you've created a deployment in Azure OpenAI Studio
...
- Ensure your API key belongs to the correct Azure OpenAI resource
- Confirm the resource is in an active state in Azure Portal

### Rate limiting

- Azure OpenAI has Tokens Per Minute (TPM) quotas per deployment
- Monitor usage in Azure OpenAI Studio under **Quotas**
...
- Check [Azure OpenAI model availability](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/models#model-summary-table-and-region-availability) for your region
