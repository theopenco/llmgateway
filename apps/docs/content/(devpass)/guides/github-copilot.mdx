...
For editor-based Copilot setup, see the separate&nbsp;[VS Code guide](https://docs.llmgateway.io/guides/vscode). BYOK works for Copilot Chat and agent mode in VS Code, but Copilot's inline code completions currently do not support BYOK or custom endpoints, including LLM Gateway.
