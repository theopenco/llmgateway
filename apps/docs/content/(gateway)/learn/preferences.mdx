...
Response caching must be disabled for every project before the organization can enable [zero data retention](/features/compliance). While ZDR is active, response caching cannot be enabled and provider prompt-cache markers are stripped.
