...
Generated responses and terminal stream parts use the negotiated finish-reason format: a string for spec v2, or `{ unified, raw }` for spec v3 and v4. The `raw` field preserves the provider's original finish reason when supplied.
