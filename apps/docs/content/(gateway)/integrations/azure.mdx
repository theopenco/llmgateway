...
<Callout type="info">
	Azure exposes three separate providers in LLM Gateway: `azure` for OpenAI
	models, `azure-ai-foundry` for third-party models served through the Azure
	Models inference endpoint, and `azure-anthropic` for Claude models on
	Microsoft Foundry. [Open an
...
Azure sells a low-latency **Priority** processing tier, requested per request via
the OpenAI-compatible `service_tier` field:
...
- A **Global Standard** or **Data Zone (US)** deployment. Regional standard and
  EU data zone deployments are not supported. Not every priority-capable model is
  offered on both — some are Global Standard only, so check Microsoft's
  [availability tables](https://learn.microsoft.com/azure/foundry/openai/concepts/priority-processing)
  for the model you deploy.
- Model version `2025-12-01` or later, on a model that offers the tier — see the
  provider card on the [models page](https://llmgateway.io/models?provider=azure).
- A subscription entitled to priority processing.

Both provider key types work — the v1 (AI Foundry) and the legacy deployment-based
surfaces each accept `service_tier` and report the tier they served.

Azure serves a priority request at standard when the entitlement is missing,
during peak load, on ramp-rate limits, and for long-context prompts on some
models. It reports the tier it actually served, so a downgraded request is
billed at standard rates and `used_service_tier` comes back `null`. See
[service tiers](/features/service-tiers) for the full behaviour.

## Claude models on Microsoft Foundry

Claude models are served by Microsoft Foundry through Anthropic's Messages API rather than an OpenAI-compatible endpoint, so they use their own `azure-anthropic` provider in LLM Gateway.

1. In the Foundry portal, deploy the Claude model you want. The deployment name is what you pass as the model at inference time, so keep the default (the model ID) unless you have a reason to rename it.
2. Add an **Azure Anthropic** provider key in LLM Gateway with your Foundry API key and the resource name from `https://<resource-name>.services.ai.azure.com`.
3. Call it with the `azure-anthropic/<model>` model string:
...
curl -X POST https://api.llmgateway.io/v1/chat/completions \
  ...
  -d '{
    ...
    "messages": [
      {
        "role": "user",
        "content": "Hello from Microsoft Foundry!"
      }
    ]
  }'
```

If a request returns `DeploymentNotFound`, the model has not been deployed in that Foundry resource, or the deployment name does not match the model string you sent. Browse the Claude models routed through this provider at [llmgateway.io/models](https://llmgateway.io/models?provider=azure-anthropic).
