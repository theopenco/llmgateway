...
- **Group → role mapping** _(optional)_ — map directory groups to organization roles (`owner` / `admin` / `project_admin` / `developer`, in priority order). On any membership change, a member receives the **highest** mapped role among their groups. Owners are never auto-demoted. Project admins still need explicit [project grants](/features/project-access).
- **Group → team mapping** _(optional)_ — map directory groups to organization teams so developers inherit team project, IAM, API-key, and budget policy. Team assignments follow group membership on every directory sync; explicit assignments made by an admin are preserved.
