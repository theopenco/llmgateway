...
- **Tool execution doesn't count.** When a completion finishes with
  ...
  the window.
- **Deploys can end very long streams early.** When a gateway instance is
  retired during a deploy, in-flight requests get up to 8 minutes to finish
  before the connection is closed. Retry a stream that ends without its final
  chunk.
