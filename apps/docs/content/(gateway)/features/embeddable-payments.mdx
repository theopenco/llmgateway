...
Your backend ──(secret key sk_)──▶ POST /v1/sessions ──▶ ephemeral session token (es_, ~15 min)
      ...
      └────────── returns es_ to your frontend ◀────────────────┘
                                  │
        Browser (es_) ──▶ chat / images / embeddings  ──▶ debits the end-user wallet
...
4. Add the browser origins allowed to call the gateway, one per line (e.g. `https://app.example.com`).
5. Under **Receipts & branding**, set your **brand name**, **support email** and **statement descriptor** so end-user payments are recognisable as yours (see below), then click **Save Settings**.
6. Under **Platform Secret Keys**, click **Create Live Key** (or **Create Test Key**) and copy the `sk_…` value immediately.
7. Store it as a server-side environment variable, for example `LLMGATEWAY_SECRET_KEY`.
...
## Receipts and statement descriptors

LLM Gateway is the **merchant of record** for every end-user top-up. Your
end-users are charged by us, and the receipt and card statement both carry our
details — which is confusing for someone who only ever visited _your_ product.
The branding fields exist to close that gap.

After a successful top-up we email the end-user a receipt with a PDF attached:

- Your **brand name** leads the email and appears on the PDF as the product the
  payment was for. It defaults to the project name.
- Your **support email**, when set, is the first contact address on the receipt.
  Ours appears below it, since we are the merchant of record and have to stay
  reachable for billing questions.
- Your **statement descriptor** is appended to our prefix on the cardholder's
  statement, giving `LLMGTWY* ACME AI` rather than a bare `LLMGTWY`. Card
  networks cap the whole string at 22 characters, so the suffix you supply is
  limited to 13 and normalized on save.

The receipt also states that LLM Gateway processed the payment and what the
charge is called on the statement. That explanation appears **only** on these
receipts — a direct LLM Gateway purchase has no third party to explain, so it
gets a plain receipt instead.

Refunds send a credit note the same way.

<Callout type="warn">
	We can only send a receipt to an address we have. Mint sessions with
	`customer: { externalId: "user_123", email: "user@example.com" }` — a bare
	`customer: "user_123"` string leaves the end customer without an email and no
	receipt is sent. Passing the email on a later session updates the stored
	record.
</Callout>

<Callout type="info">
	Test-mode wallets never receive receipts, matching Stripe's own behaviour of
	not emailing receipts for sandbox payments.
</Callout>
...
<Callout>
	**Test mode.** A `sk_test_…` key is a sandbox key: end-user wallet top-ups go
	...
	Test-mode top-ups never receive the developer-funded top-up bonus, and a
	`sk_test_…` key cannot manage webhook endpoints or use Stripe Connect
	onboarding/payouts (both return `403` — use a live secret key).
</Callout>

<Callout type="warn">
	The platform secret key is shown only once. Do not put it in frontend code,
	browser bundles, mobile apps, or public repos. Only a keyed HMAC-SHA-256
	fingerprint and masked preview are stored, so create a replacement if you lose
	the secret.
</Callout>
...
export async function POST() {
	...
	// publishableKey is null unless the project has an active publishable
	// browser key — the browser authenticates with the sessionToken (es_…).
}
```
...
- **Ephemeral tokens** (`es_…`) are short-lived and revocable; mint them per-user from your backend. Platform secrets and ephemeral tokens are stored only as keyed fingerprints, never as retrievable plaintext.
