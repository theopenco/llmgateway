...
The node adds one short round trip before the upstream call, and only routes that contain a `classifier` node pay for it. A [sticky session](/features/routing#sticky-session-routing) is rated once and the rest of the conversation reuses that verdict, so a multi-turn chat is not re-rated per turn.
