...
Dynamic routes let you move routing logic out of your application code and into the gateway. Instead of hardcoding a model, you define a named decision graph — conditions on the request, classifier verdicts, percentage-based traffic splits, and model targets — and invoke it by putting `dynamic/<name>` in the `model` field of a chat request. Dynamic routes are resolved on `/v1/chat/completions` and the endpoints that proxy through it (`/v1/responses`, `/v1/messages`, and the AI SDK gateway endpoints under `/v*/ai`) — not on embeddings, images, audio, video, rerank, OCR, or moderations:
...
Rates the request with a classifier and branches on one of its answers. Set `kind` to the classifier to use (currently `jev`, TypeSafe's decision model) and `on` to the answer to branch on:

| `on`         | Values                                                                                                                    |
| ------------ | ------------------------------------------------------------------------------------------------------------------------- |
| `difficulty` | `low`, `medium`, `high`                                                                                                   |
| `task`       | `coding`, `math`, `analysis`, `writing`, `extraction`, `summarization`, `translation`, `conversation`, `agentic`, `other` |
| `outputType` | `short_answer`, `long_form`, `code`, `structured_data`                                                                    |
...
The node adds one short round trip before the upstream call, and only routes that contain a `classifier` node pay for it. Each rating is billed at the catalogue rate for [`jev-1.13.0`](https://llmgateway.io/models/jev-1.13.0/typesafe) — roughly $0.0001 per call — and recorded as its own log entry against the same project and API key as the request that triggered it; see [what the classifier costs](/features/routing#what-it-costs). A [sticky session](/features/routing#sticky-session-routing) is rated once and the rest of the conversation reuses that verdict, so a multi-turn chat is not re-rated per turn.

`else` covers both outcomes you need it to: no case matched, **and** no verdict was produced. The classifier is fail-open — an outage, a timeout, a missing credential, or a compliance policy that blocks its provider all take the `else` branch rather than failing the route. Give `else` a sensible default model rather than an `end` node.

The verdict is recorded on the request log under `routingMetadata.dynamicRoute.classifier`, so you can see which branch was taken and why.
...
A `model` node can also target one of your organization's [custom catalog models](/features/custom-providers#custom-model-catalog) as `<customProviderName>/<modelName>`. Such a node must not set `providers` (the custom model already fixes its provider), and the referenced custom model must be active in your organization when the route is saved or published.
...
- a `classifier` node's case values must be valid for its `on` answer, and must not repeat
...
The project's **Analytics** page compares each route's spend with sending every request to the priciest model in the route; see&nbsp;[routing savings](/features/routing#routing-savings).
