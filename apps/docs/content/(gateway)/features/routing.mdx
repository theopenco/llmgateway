...
- `x-llmgateway-smart-model`: the resolved model.
- `x-llmgateway-smart-effort`: the applied effort.
- `x-llmgateway-smart-change`: a short reason, on the turn where the model or effort changed.

#### Routing metadata

Every smart-routed request made against a configured list records a `smartRouting` block on its log entry, visible in the request detail view: the classifier used, the eligible and surviving candidate models, the difficulty, task and output type, the classifier's preferred model and confidence, the band that was served, the selected model and effort, the classifier latency and cost, whether the classifier failed open, and whether the verdict was reused from earlier in the session. For a session it also records what triggered the check (`initial`, `reused`, `mid-turn`, `cache-expired` or `scan`), the recheck's view of how the work changed, why a recheck kept the current choice, and any switch with its reason and cost estimates.

## Routing Savings

The **Routing savings** card on a project's **Analytics** page (and org-wide on **Organization → Analytics** for enterprise admins) shows what `auto`, `smart` and [dynamic route](/features/dynamic-routes) requests cost compared with routing every one of them to the priciest model the router could have picked.
...
- **`auto`** — the built-in model set.
- **`smart`** — the models left on your configured list after filtering for the request.
- **Dynamic routes** — every catalogue model node in the route version that served the request. Custom-provider targets have no catalogue price and are left out.
...
<Callout type="info">
	The saving is an estimate: a different model would have produced a different
	number of output tokens. Requests from the 30 days before this feature
	launched were priced once, retroactively, at the catalogue prices in effect at
	launch.
</Callout>
