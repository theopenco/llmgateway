...
The same page exists in each project's settings. By default a project uses the organization guardrails, and the page shows them read-only.

Turn off **Use organization guardrails** on the project page to give that project its own configuration and custom rules, which then replace the organization's for that project's traffic. The organization page lists any projects that override it, so it is always clear when these settings do not apply everywhere.
