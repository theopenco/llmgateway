...
DevPass plans include a [weekly fair-use allowance](/learn/model-categories) for premium frontier models. A **Reset Pass** restores that full allowance the moment you redeem it — no waiting for the rolling 7-day window — and a fresh window starts with your next premium request. The passes live on the DevPass dashboard directly under the premium allowance meter: each pass you hold fills a slot on the strip next to the held-passes count.

<ThemedImage
	alt="DevPass usage page with the premium allowance meter and the Reset Passes row holding an included and a purchased pass"
...
Two kinds of passes appear on the strip:
