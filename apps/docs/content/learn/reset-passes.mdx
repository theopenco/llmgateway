...
- **Purchased passes** are one-time purchases at the tier's price. They never expire — they survive tier changes and even a cancelled plan, so an unused pass applies again if you resubscribe.
...
**Buy a pass** opens a one-time Stripe Checkout — no subscription change, no proration. Each purchase emails a PDF invoice and appears in the DevPass billing history alongside plan charges.
