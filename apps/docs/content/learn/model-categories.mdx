...
Both `auto` and `price` routing account for cached input costs using your project's recent model usage once enough history exists. This helps sessions that repeatedly reuse context choose providers with lower expected token costs. See [how routing learns your token mix](/features/routing#smart-routing-algorithm) for the sample requirements, defaults, and override behavior.
