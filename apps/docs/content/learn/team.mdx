...
On the Enterprise plan, a developer can be marked as a **lead** on any project they have been granted. Use **Manage access** on the member, then tick the project under **Team lead**.

A lead keeps the developer role everywhere else — no billing, no provider keys, no other projects — but gains the per-member cost view for the project they lead: that project's [Analytics](/learn/analytics) page with **Breakdown by user**. This is the way to let a team lead watch their own team's spend without promoting them to admin.
