...
can be generated at the Flex rate. In Lounge, the Image Studio shows a
**Flex** toggle whenever the selected model offers the tier.
...
- **Azure** (`azure`) — sent as a `service_tier` field in the request body on the
  ...
  entitled to priority processing. Azure downgrades to standard when the
...
- A `priority` request that runs as priority is billed at that provider mapping's priority multiplier, shown on the [model page](https://llmgateway.io/models).
