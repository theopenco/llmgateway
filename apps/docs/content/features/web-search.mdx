...
<Callout type="warn">
	**Perplexity Sonar changes on September 25, 2026.** Perplexity retires its
	...
	announcement](https://llmgateway.io/blog/perplexity-sonar-api-retirement).
</Callout>
...
Restrict which domains the model may search with `allowed_domains`, or exclude domains with `blocked_domains` (use one or the other, not both). The filters are currently forwarded to Anthropic models only; other providers ignore them:
