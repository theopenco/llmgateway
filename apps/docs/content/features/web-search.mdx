...
Restrict which domains the model may search with `allowed_domains`, or exclude domains with `blocked_domains` (provider-dependent; use one or the other, not both):
