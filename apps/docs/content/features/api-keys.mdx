...
The same rule types can also be configured **per organization member** by owners and admins on the [Team page](/learn/team). Member-level rules act as an organization-wide ceiling for that member:
...
- Key rules can only **narrow** access further — they can never grant anything the member's rules deny. For example, if an admin restricts a member to `allow_providers: ["openai"]`, the member can create a key rule allowing only a specific OpenAI model, but a key rule allowing another provider has no effect.
