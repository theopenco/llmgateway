...
For these cache-relevant requests, the price each provider is ranked on also reflects what a cached workload actually pays: the input price is blended with the provider's cached input price at an assumed cache-hit rate (`thresholds.cacheHitRate`, default `0.7`), and output is weighted at an assumed output:input ratio (`thresholds.cacheOutputRatio`, default `0.2`) instead of parity, since large-prompt traffic is dominated by input tokens. Providers that do not publish a cached input price are ranked at their full input price, matching how they bill. Setting `cacheHitRate` to `0` and `cacheOutputRatio` to `1` restores pure list-price ranking.
...
Sticky requests never enter the cross-provider [automatic retry & fallback](#automatic-retry--fallback) loop — a transient failure is retried only against the pinned provider's other configured keys. The failure still degrades that provider's uptime metrics, which is what triggers re-pinning on a subsequent request once the uptime threshold is crossed.
...
| **Thresholds**          | Cache prompt-size threshold, uptime-penalty threshold, exploration rate, and the assumed defaults used when no metrics exist                                                         | `cachePromptTokens 5000`, `cacheHitRate 0.7`, `cacheOutputRatio 0.2`, `uptimePenalty 95`, `defaultUptime 100`, `defaultLatency 1000`, `defaultThroughput 50`, `explorationRate 0.01` |
