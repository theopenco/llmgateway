...
By default, model-ID routing uses the full weighted score described above (`routing: "auto"`). When you care about a single dimension, set the `routing` field to bias provider selection toward it:

| Strategy           | Behavior                                                                                           |
| ------------------ | -------------------------------------------------------------------------------------------------- |
| `auto` _(default)_ | Full weighted smart-routing score (price, uptime, throughput, latency, cache).                     |
| `cheapest`         | Gives price a **90% relative weight**, so the cheapest provider almost always wins.                |
| `fastest`          | Gives throughput a **90% relative weight**, so the highest-throughput provider almost always wins. |

Both `cheapest` and `fastest` keep a small (10%) uptime weight, and the [exponential uptime penalty](#smart-routing-algorithm) still applies on top. This means the dominant pick is still skipped in favor of another provider when it has extremely bad uptime — you get the cheapest (or fastest) provider that is actually healthy, not one that is effectively down.
...
<Callout type="info">
	The `routing` field only affects models served by more than one provider — it
	has no effect when you pin a specific provider (e.g. `openai/gpt-4o`). On
	**coding (dev) plans**, only `auto` and `cheapest` are allowed; `fastest`
	returns a `400` error because it would bypass the prompt-cache–aware routing
