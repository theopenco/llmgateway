...
Because the choice is deterministic per session, sticky requests **bypass both the weighted scoring and the 1% epsilon-greedy exploration** — a session is never randomly bounced to a different provider mid-conversation.
