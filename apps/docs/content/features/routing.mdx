...
### Configurable auto routing (Enterprise)

Enterprise organizations can replace the built-in candidate set with their own. Under **Organization settings → Auto Routing** you choose up to 30 models from the [models catalogue](https://llmgateway.io/models) that `"model": "auto"` may resolve to, plus the classifier that ranks them. Individual projects can override the organization default on their **Settings → Routing** page; a project without an override inherits it. Owners and organization admins can edit the organization default, project admins the project override.

The configured list is exhaustive. If no model on it can serve a request — for example an image request against a text-only list — the gateway returns a 400 rather than falling back to a model you did not allow.
