...
If you need to bypass this protection and always use the exact provider you specified regardless of its current uptime, you can disable fallback using either:

1. The `routing.fallback` parameter in the request body (recommended)
2. The `X-No-Fallback` header

**Using routing.fallback (recommended):**
...
<Callout type="warn">
	Disabling fallback means your requests will be sent to the specified provider
	even if it is experiencing issues, which may result in higher error rates.
</Callout>

When fallback is disabled, the routing metadata in logs will include `noFallback: true` to indicate that fallback was disabled for that request.

## Automatic Retries

LLMGateway automatically retries failed requests to upstream providers with exponential backoff. This helps improve reliability when providers experience temporary issues.

### Default Behavior

By default, requests are retried up to **3 times** with exponential backoff delays:
...
- **Network errors**: Connection refused, DNS failures, connection resets
...
You can customize the number of retries for each request using the `routing.retries` parameter:

```bash
# Use 5 retries instead of the default 3
curl -X POST "https://api.llmgateway.io/v1/chat/completions" \
  -H "Authorization: Bearer $LLM_GATEWAY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
To disable retries for a specific request, set `routing.retries` to `0` or `null`:
...
<Callout type="info">
	Setting `routing.retries` to `null` has the same effect as setting it to `0` —
	both disable retries entirely.
</Callout>

### Combining Routing Options

You can combine multiple routing options in a single request:

```bash
# Disable fallback and use custom retry count
curl -X POST "https://api.llmgateway.io/v1/chat/completions" \
  -H "Authorization: Bearer $LLM_GATEWAY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
