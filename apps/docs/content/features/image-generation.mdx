...
| `model`           | string  | `"auto"`      | The model to use (e.g., `gemini-2.5-flash-image`, `gemini-3-pro-image-preview`) |
| `n`               | integer | `1`           | Number of images to generate (1-10)                                             |
| `size`            | string  | `"1024x1024"` | Image dimensions (e.g., `1024x1024`, `1792x1024`, `1024x1792`)                  |
| `quality`         | string  | `"standard"`  | Image quality: `standard`, `hd`, `low`, `medium`, `high`                        |
...
for (const image of response.data) {
	if (image.b64_json) {
		...
		writeFileSync("generated-image.png", buf);
	}
}
```
...
Use the `@llmgateway/ai-sdk-provider` with `experimental_generateImage`.
...
const result = await generateImage({
	model: llmgateway.image("gemini-2.5-flash-image"),
	prompt:
...
You can customize the generated image using the optional `image_config` parameter. The supported parameters vary by provider.
...
| -------------- | ------ | ---------------------------------------------------------------------------------------------------------- |
| `aspect_ratio` | string | The aspect ratio of the generated image. Examples: `"1:1"`, `"16:9"`, `"4:3"`, `"5:4"`                     |
