...
| `service_tier`    | string  | —            | Processing tier for mappings that offer one: `flex`, `priority`, or `default` — see [Service Tiers](/features/service-tiers) |
...
| `service_tier`       | enum                     | no       | `flex`, `priority`, or `default` — see [Service Tiers](/features/service-tiers)   |
...
A request stopped by the [gateway content filter](/resources/error-handling#gateway-content-filter) returns `finish_reason: "content_filter"` on the Chat Completions API and an empty `data` array on the Images API.
...
reported usage. See the [models page](https://llmgateway.io/models?filters=1&imageGeneration=true)
for current pricing.
