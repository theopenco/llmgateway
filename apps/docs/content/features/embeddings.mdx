...
- **Google Vertex AI** — `gemini-embedding-001`, `text-embedding-005`, `text-embedding-004`, `text-multilingual-embedding-002`. Prefix the model id with `google-vertex/` (e.g. `google-vertex/gemini-embedding-001`) when the same id also exists on `google-ai-studio`. `gemini-embedding-001` accepts only one input per request on Vertex; the `text-embedding-*` family supports native batching.
