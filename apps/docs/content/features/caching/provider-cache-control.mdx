...
When [Smart Routing](/features/routing) selects a provider for a large prompt (≥ 5,000 estimated tokens) or a new session, it favors providers that advertise cache support and ranks price on what a cached workload pays: uncached and cached input prices blended at your project's observed cache-hit rate from the last 24 hours, falling back to defaults until enough usage exists. See [Prompt Caching and Token Costs](/features/routing#smart-routing-algorithm) for thresholds and overrides.
...
- [Smart Routing](/features/routing) — how cache support and observed cache-hit rates influence provider selection
