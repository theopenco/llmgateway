...
On Anthropic and Vertex Anthropic, markers are forwarded on `system` blocks, message text blocks, and tool definitions. A `cache_control` on an image or other non-text content block is not forwarded — put the breakpoint on an adjacent text block instead.
