...
<Callout type="warning">
	Gateway caching is not available on DevPass coding-plan organizations or while
	a [zero data retention](/features/compliance) policy is active — the project
	setting is ignored there and every request goes upstream.
</Callout>
