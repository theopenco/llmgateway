...
- **Moderation still applies.** The output is subject to the provider's content moderation. Blocked generations finish as `failed` and are logged with a `content_filter` finish reason. The [gateway content filter](/resources/error-handling#gateway-content-filter) may also reject the request with a `403` before a job is created.
