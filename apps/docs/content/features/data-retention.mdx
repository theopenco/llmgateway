...
Most providers (including OpenAI, Azure, and Google) already count reasoning tokens inside the output token count, so each token is billed exactly once. Reasoning tokens are only added separately for providers that report them outside the output count.
