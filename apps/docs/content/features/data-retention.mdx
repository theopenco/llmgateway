...
Only organization **owners** can change the retention level — admins receive a `403`.
...
Data is retained for 30 days for all users, regardless of plan. After the retention period expires, data is automatically deleted.
...
<Callout type="info">
	...
	false` with the request to opt out. When an Enterprise organization's
	[zero-data-retention policy](/features/compliance) is active, the gateway
...
- Data is automatically deleted after the retention period (on self-hosted deployments, only when the cleanup job is enabled via `ENABLE_DATA_RETENTION_CLEANUP=true`)
