...
The gateway determines the client IP from the first header that is set, in this order:

1. `CF-Connecting-IP` — set by Cloudflare if the request transits Cloudflare.
2. `X-Forwarded-For` — first hop. This is the header set by the GCP load balancer in our deployment.
