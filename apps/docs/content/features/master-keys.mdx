...
| `providerCacheControlMode` | `"auto" \| "passthrough" \| "off"`    | [Provider cache writes](/features/caching/provider-cache-control) |
| `mode`                     | `"api-keys" \| "credits" \| "hybrid"` |                                                                   |
| `status`                   | `"active" \| "inactive"`              | Toggle the project without deleting                               |

Response (200): the updated project. It carries both `providerCacheControlMode` and the older `providerCacheControlEnabled` boolean, which reports `false` only for `"off"`.

### Delete a project

`DELETE /v1/master/projects/{id}`

Soft-deletes a project (sets `status` to `"deleted"`). Cascades to its API keys. Mirroring the dashboard's owner-only project deletion, this endpoint returns a `403` unless the user who created the master key is currently an **owner** of the organization.
...
<Callout type="info">
	The managed Playground API key cannot be updated or deleted via the master
	API.
</Callout>
...
Custom providers are your own OpenAI-compatible endpoints, registered as BYOK provider keys with `provider: "custom"`. The gateway routes to them with the model string `<name>/<model>` (e.g. `acme/acme-large`). See [Custom Providers](/features/custom-providers) for the dashboard flow and routing semantics.
	...
	"customProviders": [
		{
			...
			"provider": "custom",
			"name": "acme",
			"baseUrl": "https://llm.acme.example.com",
...
curl -X PATCH https://internal.llmgateway.io/v1/master/custom-providers/pk_... \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "baseUrl": "https://llm2.acme.example.com",
