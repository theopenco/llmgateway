...
For the [Anthropic Messages endpoint](/features/anthropic-endpoint) (`/v1/messages`), the session key is derived automatically from `metadata.user_id` — coding agents such as Claude Code embed a `session_...` id there, which is extracted and used as the session id. An explicit `x-session-id` header still takes precedence.
