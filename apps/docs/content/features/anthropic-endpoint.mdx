...
If [gateway caching](/features/caching/gateway-caching) is enabled on the project, a repeated identical request (same resolved provider and model, messages, and parameters — the cache key is built from the parsed fields, so JSON formatting outside string values doesn't matter, but a request routed to a different provider is a cache miss) is replayed from cache instead of being sent upstream. Because the replayed body carries the same `id`, content, and token counts as the original (only gateway cost fields are zeroed), the `x-llmgateway-cache: HIT` response header is the marker to check. Send `x-no-cache: true` to bypass the cache for a single request.
