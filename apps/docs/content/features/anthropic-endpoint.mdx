...
<Callout type="warning">
	Each Claude model has a minimum cacheable prompt length (1,024–4,096 tokens
	depending on the model, exposed as `min_cacheable_tokens` on `/v1/models`). A
	...
	Control](/features/caching/provider-cache-control) for details.
</Callout>
...
If [gateway caching](/features/caching/gateway-caching) is enabled on the project, a repeated identical request (same model, messages, and parameters — the cache key is built from the parsed fields, so formatting differences like whitespace don't matter) is replayed from cache instead of being sent upstream. Because the replayed body carries the same `id`, content, and token counts as the original (only gateway cost fields are zeroed), the `x-llmgateway-cache: HIT` response header is the marker to check. Send `x-no-cache: true` to bypass the cache for a single request.
