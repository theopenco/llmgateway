...
<Callout type="info">
	...
	templates/embeddable-credits**](https://github.com/theopenco/llmgateway-templates/tree/main/templates/embeddable-credits).
</Callout>
...
## Create a platform secret key

Embeddable sessions use a project-scoped **platform secret key** that starts with `sk_`. This is different from a regular gateway API key (`llmgtwy_…`) and must only be used from your backend.

1. Open the LLM Gateway dashboard and select the project you want to embed.
2. Go to **Settings → Embeddable**.
