...
Enabling **Zero data retention (ZDR)** also sets the organization's data retention to **Metadata Only** and forces `store: false` on Responses API requests. This prevents LLM Gateway from storing prompt and response payloads while the policy is active. Disable ZDR before turning payload retention back on.
