...
Once a policy is saved and enabled, owners and admins can configure **Compliance Alerts** at the bottom of the Compliance page:

- **Watched models**: pick models that no compliant provider serves yet. When a compliant provider starts serving one, LLM Gateway notifies you once, including when your own policy change makes it available. If the model later loses its last compliant provider, the watch re-arms.
- **Downgrades**: get notified when a provider stops meeting your policy (for example, it loses a certification or starts training on API data) or a watched model stops being available. The alert lists the models your organization used on that provider in the last 30 days that no longer have a compliant provider. Changes to your own policy never trigger downgrade alerts.
- **Recipients**: choose the lowest role that receives alerts — owners only, owners and admins, or everyone in the organization. Higher roles are always included. Watching the first model saves the default: owners and admins, with in-app and email delivery.
- **Delivery**: in-app, email, and Slack. Each recipient can turn off in-app or email delivery for their own account under the notification settings in the dashboard bell.

Alerts are checked every minute against the saved policy.

### Slack

Create a Slack [incoming webhook](https://api.slack.com/messaging/webhooks) for the channel that should receive alerts, and paste its URL under **Notification Channels** on the organization **Preferences** page. Use **Send test message** to verify it, then enable Slack in the Compliance Alerts card. The webhook URL is stored encrypted and only shown masked.
