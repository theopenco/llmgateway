...
It is sent as `safety_identifier` to OpenAI and Azure OpenAI, and as `metadata.user_id` to Anthropic (including Claude on Vertex AI). Google's Gemini APIs have no equivalent field, so nothing is sent there. Any `safety_identifier` you set on a request yourself is ignored — the value is always the one derived from your organization.
