...
Before enabling **Zero data retention (ZDR)**, set the organization's data retention to **Metadata Only**. While ZDR is active, **Retain All Data** is unavailable, the gateway response cache is bypassed, and Responses API requests are forced to `store: false`. Disable ZDR before turning payload retention back on.
