---
title: LLM SDK
description: Embed AI and in-app credit purchases into your app like Stripe + Stripe Elements. Your end-users get their own wallet, buy credits, and chat with any model — billed through LLM Gateway, with your markup as margin.
icon: Wallet
---
...
<Callout type="info">
	A complete, runnable Next.js example lives in the templates repo: [**LLM SDK
	credits
...
<Callout>
	**Test mode.** A `sk_test_…` key is a sandbox key: end-user wallet top-ups go
	...
	models return a `403`. Pair a test secret key on your backend with `test` on
	`<LLMGatewayProvider>` (see below) — the two must match.
</Callout>
...
<Callout type="warn">
	The frontend `test` flag and the backend secret key must be in the same mode.
	A `sk_test_…` key creates the top-up PaymentIntent in the Stripe sandbox, which
	only a sandbox publishable key (`test`) can confirm — mixing a test key with a
	live provider (or vice versa) makes `<BuyCredits>` fail to confirm.
</Callout>
...
➡️ [**LLM SDK credits template**](https://github.com/theopenco/llmgateway-templates/tree/main/templates/embeddable-credits)
