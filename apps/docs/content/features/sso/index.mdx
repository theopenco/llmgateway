...
- **Default project access** _(optional)_ — choose which projects SSO/SCIM-provisioned members (the `developer` role) get when none of their groups map to projects. Owners/admins always see every project, so this and the group mapping only affect developers. Left unconfigured, members get the organization's first (default) project.
