---
title: SSO
description: Per-organization SAML single sign-on and SCIM directory provisioning (Enterprise, or as Pro plan add-ons).
icon: ShieldCheck
---
...
<Callout type="info">
	SSO & SCIM are available on the **Enterprise** plan and as paid add-ons on the
	**Pro** plan (SSO, plus SCIM as a separate add-on on top — for organizations
	with up to 100 seats; larger teams need Enterprise), and can be configured by
	an organization **owner** or **admin**. Until the organization has SSO access,
	the **SSO** page shows an upgrade card. Pro organizations can enable the
	add-ons from the Plan page; for Enterprise, contact us at
...
2. **The organization has SSO access.** Either it is on the Enterprise plan, or it is a Pro organization with the SSO add-on enabled on its subscription. SCIM token management additionally requires the SCIM add-on.
