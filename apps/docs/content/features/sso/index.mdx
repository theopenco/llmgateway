---
title: SSO
description: Per-organization SAML single sign-on and SCIM directory provisioning (Enterprise, or as a Pro plan add-on).
icon: ShieldCheck
---
...
<Callout type="info">
	SSO & SCIM are available on the **Enterprise** plan and as a paid add-on on
	the **Pro** plan, and can be configured by an organization **owner** or
	**admin**. Until the organization has SSO access, the **SSO** page shows an
	upgrade card. Pro organizations can enable the add-on from the billing page;
	for Enterprise, contact us at contact@llmgateway.io.
</Callout>
...
2. **The organization has SSO access.** Either it is on the Enterprise plan, or it is a Pro organization with the SSO & SCIM add-on enabled on its subscription.
