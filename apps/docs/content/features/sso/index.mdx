...
- **Group → project access mapping** _(optional)_ — map directory groups to project access. Members of a mapped group are granted exactly the mapped projects **instead of** the default set below, and the grants are re-synced on every directory sync — removing someone from a group revokes the mapped projects automatically. Project access granted manually on the **Team** page is never touched by the sync.
- **Default project access** _(optional)_ — choose which projects SSO/SCIM-provisioned members (the `developer` role) get when none of their groups map to projects. Owners/admins always see every project, so this and the group mapping only affect developers. The saved selection is authoritative — saving an empty selection means members get **no** project access until you pick defaults or group mappings. Organizations created before this setting existed and that never saved it keep the old fallback (the organization's first project).
