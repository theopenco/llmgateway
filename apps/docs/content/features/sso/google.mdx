...
With a Google-only setup, the **Directory sync (SCIM)**, **Group role mapping**, and **Group project access** cards on the SSO page are disabled:
...
- **Group project access** — group → project mappings likewise rely on SCIM-pushed groups. Grant projects per member on the **Team** page instead.
