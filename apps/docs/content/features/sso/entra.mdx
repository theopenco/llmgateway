...
In LLM Gateway (**Organization → SSO & SCIM → Add a connection**), pick a **connection slug** (lowercase letters, numbers, hyphens; `^[a-z0-9-]+$`, up to 64 chars), e.g. `acme-entra`. The form suggests one from your organization name — accept it or edit it. It must be unique across all LLM Gateway organizations, and each organization has exactly one SSO connection. Keep it stable: your SP URLs are built from it.
