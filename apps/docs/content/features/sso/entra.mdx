...
Group-based project sync rides entirely on the SCIM setup above — there is no extra Entra credential to configure — but three Entra-side details determine whether the group data actually reaches LLM Gateway:

1. **Group provisioning must be on** (it is by default). On the Enterprise App → **Provisioning → Attribute mapping**, the **Provision Microsoft Entra ID Groups** mapping must be enabled. Entra then pushes each in-scope group's display name and member list over SCIM.
2. **The group must be in the provisioning scope.** With the recommended **"Sync only assigned users and groups"** scope, assign the group itself under **Users and groups** on the Enterprise App. Assigning a group both provisions its members as users and pushes the group object used for the mapping.
3. **The mapping's group name must match the Entra display name** exactly (e.g. `Data Science`, not a case-variant). If you rename the group in Entra, the rename is pushed on the next sync and members are re-evaluated against the mappings — update the mapping to the new name if you want it to keep applying.
