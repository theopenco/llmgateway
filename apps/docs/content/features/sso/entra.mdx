...
## Group → team mapping (optional)

The **Group team mapping** card on the **SSO** page maps an Entra group to an organization team. Developer members then inherit that team's project, IAM, API-key, and budget policy.

- Enter the group's exact **display name**, select a team, and save. Saving the same group again replaces its team.
- Assign the group itself to the Enterprise App and leave group provisioning enabled under **Provisioning → Attribute mapping → Groups**. Assigning only individual users provisions the users but does not push the group object or its membership.
- Assignments are recomputed on provisioning and reactivation, membership changes, group renames and deletions, and immediately when a mapping is saved or deleted. Leaving the last mapped group removes a SCIM-assigned team.
- A team explicitly assigned by an admin on the **Members** page is never overwritten. If an admin only removes a synced assignment, the next SCIM event restores the mapped team.
- A developer can belong to one organization team. If several mapped groups apply, the alphabetically first exact group name wins. A group → role mapping that makes the member an owner or admin takes precedence, because privileged members cannot belong to teams.
...
<Callout type="info">
	To test without waiting for Entra's provisioning cycle, use **Provision on
	demand** with the group and select the affected member. Provisioning only the
	user does not push group membership.
</Callout>

<Callout type="warn">
	If the mapped group is a user's only assignment to the Enterprise App,
	removing them from it can deprovision the user entirely instead of only
	removing their team. Assign users through a broader group or directly when
	group leave should only change policy.
</Callout>
