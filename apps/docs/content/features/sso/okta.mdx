...
Provisioned members join as **developers**, who only see the projects they're granted (owners/admins always see every project). The **Group project access** card on the **SSO** page maps an Okta group to one or more projects, so project access follows group membership in your directory automatically.
...
On the app's **Push Groups** tab, push the Okta groups whose membership should drive project access (the same mechanism role mappings use), then add mappings such as `Data Science → Analytics, ETL` on the **SSO** page. The sync behaves as follows:

- **Mapped projects replace the default set.** A member of at least one mapped group gets exactly the union of their groups' mapped projects; members in no mapped group fall back to the [Default project access](#default-project-access-optional) selection below.
- **Grants are re-synced on every directory sync event** — provisioning, reactivation, group membership pushes, group renames/deletions, and immediately when you create or delete a mapping for an already-pushed group. Removing a user from a mapped group revokes the mapped projects on the next push.
- **Manually granted access always survives.** Project access granted on the **Team** page is never removed by the sync; conversely, manually removing a synced grant only lasts until the next directory sync re-applies the mapping.
- Mappings match on the pushed group's **name** exactly. Saving a mapping for a group that already has one **replaces** that group's project set.

<Callout type="warn">
	If the mapped group is also the user's **only** assignment to the Okta app,
	removing them from it unassigns them entirely — Okta deprovisions the user and
	their LLM Gateway membership and API keys are revoked, not just the mapped
	project. Assign users via a broader group or directly if group-leave should
	only adjust project access.
</Callout>
