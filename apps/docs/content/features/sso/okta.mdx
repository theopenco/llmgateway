...
Push each relevant group from the app's **Push Groups** tab, then use the **Group team mapping** card on the LLM Gateway **SSO** page to map its exact name to an organization team. Developer members inherit that team's project, IAM, API-key, and budget policy.

- Assignments are recomputed on provisioning and reactivation, group membership pushes, renames and deletions, and immediately when a mapping is saved or deleted.
- Explicit team assignments made by an admin are preserved. Removing only a synced assignment is temporary; the next SCIM event restores the mapping.
- A developer can belong to one team. If several mapped groups apply, the alphabetically first exact group name wins. Owners and admins cannot belong to teams, so a privileged role mapping takes precedence.
- Team project access intersects with the member's personal/default project grants; both must allow a project.

<Callout type="warn">
	If the mapped group is also the user's only assignment to the Okta app,
	removing them can deprovision the user and revoke their API keys. Assign users
	through a broader group or directly when group leave should only change team
	policy.
</Callout>
