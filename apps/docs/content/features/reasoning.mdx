...
- Anthropic's Claude 3.7 Sonnet, Claude Sonnet 4, and Claude Opus 4 / 4.5 / 4.6 / 4.7 / 4.8
...
<Callout type="info">
	**Adaptive reasoning (Claude Opus 4.6 / 4.7 / 4.8).** These models use
	Anthropic's adaptive thinking, where the model itself decides how much to
	think based on the difficulty of the prompt. Control the depth with
	`reasoning_effort` (or `reasoning.effort`) — `low`, `medium`, `high`, `xhigh`,
	or `max`. A `reasoning.max_tokens` value is still accepted on these models,
	but it is routed to adaptive thinking rather than enforced as an exact budget.
	For easy prompts the model may reason only briefly (or skip visible reasoning)
	and begin its answer almost immediately even at `high` effort — this is
	expected adaptive behavior, not a disabled-reasoning bug.
</Callout>
