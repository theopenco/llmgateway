...
The **Incidents** page lists every mapping of your claimed providers that failed requests in the selected window (1 hour to 7 days), with its error rate, error count, upstream/gateway split, and total requests. Client errors — requests the caller got wrong — are excluded.
