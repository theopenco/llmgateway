...
Before a listing can be filed, Airside runs a **preflight** against your endpoint through that API: one check per declared capability, using a provider key that is used only for that run and erased when it finishes. Editing the model invalidates the run, so a listing never ships with checks made against different settings.
