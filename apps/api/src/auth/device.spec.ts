...
describe.each(["llmgateway-cli", "llmgateway-lounge-ios"])(
	"%s device authorization",
	(clientId) => {
		...
		test("requires explicit browser approval, returns a usable bearer session, and consumes the code once", async () => {
			...
			// Signing the device out does not revoke the browser's independent session.
			expect(
		...
		test("a bearer session takes precedence over a browser cookie", async () => {
			...
			expect(await session(otherToken)).toMatchObject({
				user: { id: otherId },
			});
		...
		test("cannot redeem another allowed client's code", async () => {
			const code = await start();
			await approve(code.user_code);
			const response = await request("/device/token", {
				client_id:
					clientId === "llmgateway-cli"
						? "llmgateway-lounge-ios"
						: "llmgateway-cli",
			...
			expect((await poll(code.device_code)).status).toBe(200);
		});
		...
		test("SSO-enforced users can authorize their device with an existing session but cannot sign in with a password", async () => {
