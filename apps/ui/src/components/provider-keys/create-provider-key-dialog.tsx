	...
	const availableProviders = providers
		...
		.map((provider) =>
			provider.id === "custom" && !isEnterprise
				? { ...provider, name: `${provider.name} (Enterprise)` }
				: provider,
		);
	const customProviderLocked = selectedProvider === "custom" && !isEnterprise;
			...
			<DialogContent className="sm:max-w-[500px]">
				...
				<form onSubmit={handleSubmit} className="space-y-4 py-4">
						...
						<>
							<div className="space-y-2">
								...
								{(() => {
									...
									const instructions = (provider as ProviderDefinition)
										?.apiKeyInstructions;
									const learnMoreUrl = (provider as ProviderDefinition)
										?.learnMore;
							...
							{selectedProvider === "azure" && (
								<>
									...
									<div className="space-y-2">
										<Label htmlFor="azure-deployment-type">
											Deployment Type
										</Label>
										<Select
									...
									<div className="space-y-2">
										<Label htmlFor="azure-validation-model">
											Validation Model
										</Label>
										<Input
							...
							{selectedProvider === "azure-ai-foundry" && (
								<>
									<div className="space-y-2">
										<Label htmlFor="azure-ai-foundry-resource">
											Resource Name
										</Label>
										<Input
											...
											onChange={(e) =>
												setAzureAiFoundryResource(e.target.value)
											}
											required
										/>
										<p className="text-sm text-muted-foreground">
									...
									<div className="space-y-2">
										...
										<Input
											...
											onChange={(e) =>
												setAzureAiFoundryApiVersion(e.target.value)
											}
										/>
										<p className="text-sm text-muted-foreground">
							...
							{selectedProvider === "azure-anthropic" && (
								<div className="space-y-2">
									<Label htmlFor="azure-anthropic-resource">
										Resource Name
									</Label>
									<Input
							...
							{usesWorkspaceId && (
								<div className="space-y-2">
									...
									<Input
										...
										onChange={(e) =>
											setAlibabaWorkspaceId(e.target.value.trim())
										}
							...
							{selectedProvider === "custom" && (
								<>
									<div className="space-y-2">
										...
										/>
										<p className="text-sm text-muted-foreground">
											Used in model names like: {customName || "myprovider"}
											/gpt-4o
										</p>
									</div>
									<div className="space-y-2">
							...
							<div className="space-y-2">
								<Label htmlFor="provider-key-usage-limit">
									Max spend (USD)
								</Label>
								...
								<p className="text-sm text-muted-foreground">
									Optional security fuse: the key is automatically disabled once
									the spend attributed to it reaches this amount.
								</p>
							</div>
						</>
					)}
					...
					<DialogFooter>
						...
						<Button
							type="submit"
							disabled={isValidating || customProviderLocked}
						>
