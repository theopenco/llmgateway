...
Kimi K3 is Moonshot AI's flagship model for long-horizon coding and agentic work. At 2.8 trillion parameters (mixture-of-experts), it is the largest open-weight model announced to date — the full weights are scheduled to publish on July 27 under a Modified MIT license.

The specs that matter in practice:

- **1M-token context window** (1,048,576 tokens), with matching max output — enough to hold a large repository plus its docs in a single request
...
If K3 is headed for your coding agent, the cheapest way to run it hard is [DevPass](https://devpass.llmgateway.io) — a flat monthly plan that works with Claude Code, OpenCode, Cursor, Cline, and anything else OpenAI- or Anthropic-compatible:
...
1,048,576 tokens — a full 1M-token context with matching maximum output, the same headline context as GLM-5.2 and DeepSeek V4 Pro.

### Can I use Kimi K3 with Claude Code or Cursor?

Yes. LLM Gateway exposes OpenAI- and Anthropic-compatible endpoints, so point your tool's base URL at `https://api.llmgateway.io/v1` with your key and select `kimi-k3` as the model. With DevPass this is covered by the flat monthly rate. Step-by-step setup for each tool: [How to Use Kimi K3 with Claude Code, Cursor, and Cline](/blog/kimi-k3-claude-code).
...
- Go deeper: [Kimi K3 vs Claude Opus 4.8](/blog/kimi-k3-vs-claude-opus), [can you run Kimi K3 locally?](/blog/run-kimi-k3-locally), and the full [open-source LLM ranking](/blog/best-open-source-llms)
