...
describe("calculateCosts", () => {
	...
	it("applies AWS Bedrock 10% regional pricing but not global pricing", async () => {
		...
		const inRegionCrossRegion = await calculateCosts(
			"claude-opus-4-6",
			"aws-bedrock",
			"eu-west-2",
			1_000_000,
		...
		expect(inRegionCrossRegion.inputCost).toBeCloseTo(5.5);
		expect(inRegionCrossRegion.outputCost).toBeCloseTo(27.5);
