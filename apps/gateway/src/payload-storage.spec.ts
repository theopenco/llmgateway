...
describe("payload storage offloading", () => {
	...
	beforeAll(async () => {
		...
		// MINIO_PORT is set by offset per-worktree stacks (see docker-compose.yml).
		process.env.PAYLOAD_STORAGE_S3_ENDPOINT = `http://localhost:${process.env.MINIO_PORT ?? "9000"}`;
