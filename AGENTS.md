...
- **Exactly one header is trusted, named by `CLIENT_IP_HEADER`, and every service refuses to start without it.** Production sets it to `X-Client-Ip`, which the load balancer writes from `{client_ip_address}` with `set` (not `add`), so a caller cannot forge it. There is deliberately **no fallback chain and no default**: trying several headers in turn means trusting whichever one a caller happened to send, which is useless for rate limiting and actively wrong for IP allow-lists. Never add candidate headers or a default back — a missing value must fail at deploy time, not silently bucket every visitor together at runtime. Every deployment needs the variable (api, gateway, ui, playground, code, admin, docs); `assertClientIpHeaderConfigured()` runs in the api and gateway `serve.ts` and in each Next.js app's `instrumentation.ts`.
