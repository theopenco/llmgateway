...
- Screenshots in a PR description are ONLY for changes to the four dashboard UIs: the main UI (`apps/ui`), the DevPass UI (`apps/code`), the admin UI (`ee/admin`), and the Airside carrier portal (`apps/airside`). For those, always embed screenshots of the affected screens — run the app locally, capture the new/changed state, and use the `pull-request` skill's screenshot workflow to attach them. For changes to an existing screen, show a before/after pair; for anything with light and dark styling, include both themes
...
- Use `formatNumber`, `formatCompactNumber`, and `formatChartValue` from `@llmgateway/shared/number-format` for displayed counts and chart values. Keep date, currency, and CSV formatting separate.
...
- Models and provider mappings already present on `origin/main` can NEVER be removed, only deactivated. To retire one, set `deactivatedAt: new Date("YYYY-MM-DD")` on the relevant provider mapping(s) instead of deleting the definition. Historical usage records and analytics reference these definitions, so deleting them breaks lookups. New definitions added only on the current unmerged branch may be removed normally.
- NEVER deactivate a mapping that still serves live requests on the same day. Set `deactivatedAt` at least 7 days out so customers pinned to that provider — or narrowed to it by a compliance policy — have time to react, and use the provider's announced retirement date when there is one. A same-day or retroactive date requires the mapping to be confirmed completely broken today: it cannot serve a live request at all. Failing one case is not that. A rejected parameter, a capability the deployment does not have, or flaky quality is a metadata fix on that mapping (`jsonOutput`, `vision`, `reasoningEfforts`, `supportedToolChoices`, `stability: "unstable"`, `test: "skip"`) — or, when the defect is on our side, a gateway fix. Leave the mapping active and say so in the PR rather than retiring it.
...
- Never write a new IP extractor, in any app. Everything lives in `@llmgateway/shared/client-ip`: `getClientIp`, `getClientIpFromHeaders`, `getClientIpFromContext`, `getClientIpFromRequest`, `getClientIpFromForwardedFor`, `isPublicIp`, `ipMatchesCidr`, `anyCidrMatches`, `forwardedIpHeaders`. If a new caller needs a variant, add it there rather than inlining a header lookup.
- **Exactly one header is trusted, named by `CLIENT_IP_HEADER`.** There is deliberately **no fallback chain**: trying several headers in turn means trusting whichever one a caller happened to send, which is useless for rate limiting and actively wrong for IP allow-lists. Never add candidate headers back. It defaults to `X-Forwarded-For`, which is correct behind a reverse proxy that overwrites it — self-hosting and local runs need no configuration. **`HOSTED=true` requires it and the service refuses to start without it** (`assertClientIpHeaderConfigured()` in the api and gateway `serve.ts` and in each Next.js app's `instrumentation.ts`): there the load balancer *appends* to `X-Forwarded-For`, so the default would be forgeable, and the failure is invisible. Hosted sets it to `X-Client-Ip`, written from `{client_ip_address}` with `set` (not `add`).
...
- Frontends reach the API over the in-cluster Service (`API_BACKEND_URL`), not the public host. A server-rendered page or Next.js proxy route that calls the API on a visitor's behalf must spread `forwardedIpHeaders(headers)` into the request, or every visitor shares the calling server's rate-limit bucket.
