...
- **`X-Client-Ip` is the only client address you may trust.** The load balancer sets it from `{client_ip_address}` on every route with a backend, and `set` (not `add`) overwrites whatever the caller sent. `X-Forwarded-For` is forgeable — the load balancer appends to the chain rather than replacing it, so its first hop is caller-supplied — and is only consulted as a fallback, where the absence of `X-Client-Ip` means the request came from inside the cluster. Never reorder that precedence.
