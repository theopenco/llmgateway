...
- Do not cache `process.env` reads or parsed env-var values unless there is a measured hot-path need; prefer simple helpers that read and parse env vars directly so runtime env changes and tests stay predictable
