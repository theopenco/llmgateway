...
- **Exactly one header is trusted, named by `CLIENT_IP_HEADER`.** Production sets it to `X-Client-Ip`, which the load balancer writes from `{client_ip_address}` with `set` (not `add`), so a caller cannot forge it. There is deliberately **no fallback chain**: trying several headers in turn means trusting whichever one a caller happened to send, which is useless for rate limiting and actively wrong for IP allow-lists. Never add candidate headers back. Every deployment that reads or forwards a client IP needs the variable — api, gateway, ui, playground, code, admin and docs — and the api and gateway log an error at startup when `HOSTED=true` and it is unset.
