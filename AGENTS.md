...
- Models and provider mappings already present on `origin/main` can NEVER be removed while in service, only deactivated. To retire one, set `deactivatedAt: new Date("YYYY-MM-DD")` on the relevant provider mapping(s) instead of deleting the definition. Historical usage records and analytics reference these definitions, so deleting an in-service one breaks lookups. A provider definition (with its mappings and adapter cases) may be deleted only once every one of its mappings is on `main` with a past `deactivatedAt`: the worker sync then marks the retained `provider` / `model_provider_mapping` rows inactive instead of deleting them, and historical reads resolve names and prices from those rows (see `getCatalogueProviderIds` in `packages/db`). New definitions added only on the current unmerged branch may be removed normally.
